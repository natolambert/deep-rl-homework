...
class Agent(object):
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            sy_sampled_ac = sy_mean + tf.multiply(z,tf.exp(sy_logstd))
    ...
    def build_computation_graph(self):
        ...
        # define the critic
        self.tuned = False # actor critic performance search
        if self.tuned:
            ...
            self.critic_update_op = tf.train.AdamOptimizer(.7*self.learning_rate).minimize(self.critic_loss)
        else:
            self.critic_prediction = tf.squeeze(build_mlp(
                                    self.sy_ob_no,
                                    1,
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # Compute q values:
        Vsprime_n = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no:next_ob_no})
        # TODO add cutoff terms
        q_n = np.zeros([len(re_n)], dtype=np.float32)
        for (i,t) in enumerate(terminal_n):
            if t == 0:
                # print(np.shape(q_n[i]))
                # print(np.shape(re_n[i]))
                # print(np.shape(Vsprime_n[i]))
                q_n[i] = re_n[i]+self.gamma*Vsprime_n[i]
            else:       # if terminal, reward does not have the reward to go
                q_n[i] = re_n[i]

        # get advantages from QQ
        Vs_n = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no:ob_no})
        adv_n = q_n - Vs_n
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # otherwise the values will grow without bound.
        # YOUR CODE HERE
        # compute initial advantage / target
        adv_n = self.estimate_advantage(ob_no, next_ob_no, re_n, terminal_n)
        # self.target_n = adv_n + self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no:ob_no})
        self.target_n = re_n + self.gamma*self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no:next_ob_no})

        # deal with terminal values:
        for (i,t) in enumerate(terminal_n):
            if t == 1:
                self.target_n[i] = re_n[i]
        ...
        for i in range(1,n+1,1):
            # Take gradient step critic_loss
            # self.sess.run(self.critic_loss,
            #     feed_dict={self.sy_ob_no: ob_no, self.sy_target_n: self.target_n})
            self.sess.run(self.critic_update_op,
                feed_dict={self.sy_ob_no: ob_no, self.sy_target_n: self.target_n}) #, self.sy_adv_n: adv_n})

            # if update number is on number of grad steps per target update...
            # update target when have taken number of grad updates
            if (i % self.num_grad_steps_per_target_update) == 0:
                # update target value
                # self.target_n = self.estimate_advantage(ob_no, next_ob_no, re_n, terminal_n) + self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no:ob_no})
                self.target_n = re_n + self.gamma*self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no:next_ob_no})
                # deal with terminal values:
                for (i,t) in enumerate(terminal_n):
                    if t == 1:
    ...
    def update_actor(self, ob_no, ac_na, adv_n):
        ...
        # For non cart pole tasks
        self.sess.run(self.actor_update_op,
            feed_dict={self.sy_ob_no: ob_no, self.sy_ac_na: ac_na.reshape(-1,self.ac_dim), self.sy_adv_n: adv_n})
...
def train_AC(
    ...
    for itr in range(n_iter):
        ...
        agent.update_actor(ob_no, ac_na.squeeze(), adv_n)
