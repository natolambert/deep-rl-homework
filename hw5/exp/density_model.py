...
class Histogram(Density_Model):
    ...
    def get_count(self, states):
        ...
        counts = np.zeros(np.shape(states)[0])
    ...
    def get_prob(self, states):
        ...
        probs = np.zeros(np.shape(states)[0])

        for (i,state) in enumerate(states):
            probs[i] = get_count(state)

        probs /= float(self.total)
...
class RBF(Density_Model):
    ...
    def fit_data(self, data):
        ...
        self.means = np.array(data) # np.repeat([data], B, axis=0)
    ...
    def get_prob(self, states):
        ...
        if self.means is None:
            ...
            deltas = np.repeat(np.expand_dims(states,1), B, axis=1) - np.expand_dims(self.means,0)
            # print(deltas.shape)
            # print(b,B,ob_dim)
            ...
            # print(np.shape(gaussians))
            assert gaussians.shape == (b, B)

            # 4. Average
            densities = (1/B)*np.sum(gaussians,axis=1)
...
class Exemplar(Density_Model):
    ...
    def make_encoder(self, state, z_size, scope, n_layers, hid_size):
        ...
        z_logstd = tf.Variable(np.zeros(int(z_size)), name="z_logstd", dtype=tf.float32)
    ...
    def make_prior(self, z_size):
        ...
        """
        prior_mean = tf.Variable(np.zeros(int(z_size)), name="prior_mean", dtype=tf.float32)
        prior_logstd = tf.Variable(np.zeros(int(z_size)), name="prior_logstd", dtype=tf.float32)
    ...
    def forward_pass(self, state1, state2):
        ...
        # Encoder: changed self.hid_dim/2 to self.ob_dim
        ...
        print("---Shape of individual z1/z2 is: ", z1.shape)
    ...
    def get_likelihood(self, state1, state2):
        ...
        # gets which states are the same for targets
        discrim_target = np.all((state1==state2),axis=1)

        likelihood = self.sess.run([self.likelihood], feed_dict={self.state1: state1,
                                            self.state2: state2,
                                            self.discrim_target: discrim_target.reshape(-1,1)})
